...
async function fetchRiwayatAdmin() {
    try {
        ...
        document.querySelector('.notificationContainer').innerHTML =
            `<p style="text-align:center; color:red; padding:2rem;">Gagal memuat data: ${error.message}</p>`;
    }
}
...
function renderHistory(data) {
    ...
    if (!data || data.length === 0) {
        container.innerHTML = '<p style="text-align:center; padding:2rem;">Belum ada riwayat bimbingan.</p>';
        return;
    }
    ...
    data.forEach(item => {
        ...
        if (item.status === 'Disetujui' || item.status === 'Selesai') statusColor = 'green';
        ...
        card.innerHTML = `
            <div class="glass-box isiHistory">
                <h2 class="textContent">${item.nama_mahasiswa || "Mahasiswa"}</h2>
                ...
                <div class="info">
                    ...
                    <p class="dotSpacing">:</p>
                    <h3 class="subContent" style="color:${statusColor}; font-weight:bold;">${item.status}</h3>
                </div>

                <div class="info" style="align-items:flex-start;">
                    <h3 class="subContent">Catatan</h3>
                    <p class="dotSpacing">:</p>
                    <h3 class="subContent" style="font-weight:normal;">${item.catatan || "-"}</h3>
                </div>
            </div>
        `;
