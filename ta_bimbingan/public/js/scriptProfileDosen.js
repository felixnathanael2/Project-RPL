...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error(`Gagal mengambil data profil Dosen. Status: ${response.status}`);
        }

        const data = await response.json();
        
        // 2. Update Data Profil UTAMA (Data yang tersedia di DB: nama, id, email)
        
        // Informasi Utama
        setText("profile-nama", data.nama);
        setText("profile-role-id", `Dosen | ${data.id_users}`); // Menggabungkan Role & nIK 
        setText("profile-email", data.email);
        
        // 3. Update Data STATIS / FALLBACK
