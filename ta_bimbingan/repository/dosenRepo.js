...
export async function getAllDosen() {
    const pool = await connectDB();
    const query = `SELECT users.nama, users.id_users
                   FROM users
                   WHERE role = 2;`;
...
export async function getMahasiswaEligible(nik) {
    ...
    const query = `
        SELECT users.nama                                       AS nama_mahasiswa,
               ...
               SUM(CASE
                       WHEN b.tanggal <= semester_aktif.tanggal_UTS_selesai AND b.status = 'Selesai' THEN 1
                       ELSE 0 END)                              AS total_bimbingan_pre_uts,

               COUNT(CASE WHEN b.status = 'Selesai' THEN 1 END) AS total_bimbingan_total
        ...
        FROM plotting_pembimbing
                 ...
                 LEFT JOIN bimbingan b ON dt.id_data = b.id_data
            AND b.tanggal BETWEEN semester_aktif.tanggal_awal_semester AND semester_aktif.tanggal_UAS_selesai
