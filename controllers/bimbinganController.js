...
export const submitBimbingan = async (req, res) => {
  try {
    const { npm, id_lokasi, hari, jam_mulai } = req.body;

    // Hardcode dulu atau ambil dari session
    const dosenId = req.user.id;
    console.log("INI NIK DOSEN", dosenId);

    if (!npm || !id_lokasi || !hari || !jam_mulai) {
      return res
        .status(400)
        .json({ success: false, message: "Data tidak lengkap." });
    }

    // Panggil Service RUTIN
    const result = await bimbinganService.submitJadwalRutin({
      npm,
      id_lokasi,
      hari,
      jam_mulai,
    ...
    return res.status(201).json({
      success: true,
      message: `Berhasil membuat ${result.total_dibuat} jadwal pertemuan rutin!`,
      data: result,
    });
  } catch (error) {
    console.error("Error submit rutin:", error);
    return res.status(500).json({
      success: false,
      message: error.message || "Gagal menyimpan jadwal rutin.",
    });
  }
};
